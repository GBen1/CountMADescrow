...
if [[ "$r" -eq 0 ]] ; then
 ...
 if [[ "$difftx" -lt 30 ]] ; then
  ...
  mkdir ../CountMADescrow/MYGRAPHS 2>/dev/null
  mkdir ../CountMADescrow/MYGRAPHS/$date 2>/dev/null
  echo -e "${gr}BLOCK ${neutre}${yel}$currentblock ${neutre}${gr}: $multisig1${neutre} ${bl}(RELEASED : $getblockmadtx1)${neutre} ${red}(POTENTIAL FAKE) ${neutre}" >> ../CountMADescrow/MYGRAPHS/$date/madlist.txt
  isfake=$(($isfake + 1))
  madlist=$(($madlist + 1)) 
  else
  echo -e "${gr}BLOCK ${neutre}${yel}$currentblock ${neutre}${gr}: $multisig1${neutre} ${bl}(RELEASED : $getblockmadtx1)${neutre} ${red}(POTENTIAL FAKE) ${neutre}" >> ../CountMADescrow/MYGRAPHS/$date/madlist.txt
